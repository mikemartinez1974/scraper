...
(async () => {
    ...
    async function getTask(){
        let url = "http://localhost:8880/task";
    ...
    async function getNextQuery(){
        let url = "http://localhost:8880/query";        
        let response = await axios.get(url);
        if(debugging) console.log(response.data);
        if (response.err) {console.log(response.err);}
    ...
    * ex: proxy6.infatica.com:2309 */
    async function getNextProxy(){
        let url = "http://localhost:8880/proxy";  
        let response = await axios.get(url);
        if(debugging) console.log(response.data);      
        if (response.err) {console.log(response.err);}
        else { return response.data};
    ...
    async function getNextAgent(){
        let url = "http://localhost:8880/agent"
        let response = await axios.get(url);
        if(debugging) console.log(response.data);
        if (response.err) {console.log(response.err);}
        else { return response.data};
    ...
    async function createRequestOptions(lastpage = true, retry = false, start = false) {
        ...
        if (start == true) 
        {
            let response = await getNextQuery();
            nextUrl = response.query;
            nextProxy = response.proxy;
            nextAgent = response.agent;
        }
        else if((retry == true))
        {
            // leave url as i is.
            let response = await getNextProxy();
            nextProxy = response.proxy;
            nextAgent = response.agent;
        }
        else
